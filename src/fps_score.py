...
def load_go(path):
    ...
    # Multi-map: gene symbol -> set(go_id)
    return go
...
def load_loeuf(path):
    ...
    if path.endswith(".gz"):
        ...
        df=pl.read_csv(path, separator="\t")
    # Try common headers: gene, gene_symbol, loeuf, lof.oe_ci.upper
    cols=df.columns
...
def exonic_density(vcf_path, exons_df):
    # Build in-memory index per chrom for quick scan (chr21 coordinates must match VCF build)
    # Using naive scan (fast enough for chr21)
    ...
    vcf=VCF(vcf_path)
    for var in tqdm(vcf, desc="VCF variants"):
        pos=var.POS
